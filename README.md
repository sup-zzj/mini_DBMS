...
### 负面结果归因小结（为什么学习式索引在这里不赢）

实验 1 / 4 / 5 共同指向同一个结论：**在这台机器的 10 万整数内存点查上，学习式索引比不过 `bisect`，也比不过 B+ 树**。这不是实现缺陷（正确性已逐 key 验证），而是三层原因叠加的结果，由大到小：

1. **Python 解释器开销（主因）**。一次 learned lookup 要依次执行：`bisect` level-1 边界路由 → 列表索引选段 → 选 piece → 一次乘加 → 再 `bisect` 窗口搜索，至少 3 层 Python 函数调用 + 数次纯 Python 运算。`bisect` 虽然也是 Python 实现，但它一次调用就完成全部比较，且循环体是高度内联的 C 代码。实测 learned ≈ 2.3 μs vs bisect ≈ 1.4 μs——多出的 ~0.9 μs 几乎全部是调用链开销。**模型省下的比较次数，少于它引入的解释器开销。**
2. **内存 cache 友好性（环境因素）**。Kraska 2018 的核心论点是"二分查找在页定位阶段会频繁 miss cache-line"。但那是**外存/磁盘页**的假设：10 万 int64 只有 0.76 MB，整棵树（含 keys 数组）驻留在 L3 cache 甚至 L2 里，二分查找根本不会 miss。**学习式索引的收益前提（缓存不友好）在内存小数据上不成立。**
3. **数据太平（数据因素）**。整数均匀分布的位置函数近似一条直线，任何模型都能"学会"——这反而让 RMI 的模型能力无用武之地：`bisect` 已经以理论最优的 `log2(N) ≈ 16.6` 次比较逼近下限，模型拟合不出更短的路。分布敏感性实验（实验 4）也印证：换更偏斜的 zipf，模型没有变快，只是把复杂度从数据搬进了模型（pieces 349 → 638）。

**30 秒面试版**：*"学习式索引声称用模型跳过二分查找，但它的前提是'定位页的开销远大于比较本身'——这是磁盘场景。我的实验证明：在内存、Python、小数据下，模型省下的比较次数抵不过它引入的调用开销，cache 又本来就友好，数据还太平，所以三个前提全不成立。真正的验证应该在 C++ + 外存 + 大 key 下做，那是我的下一个实验。"*

**结论的反转条件**：三层原因都指向同一个边界——**当实现语言引入的常数小于被省掉的比较次数、当数据大到 cache 容不下、当 key 分布复杂到值得建模时**，学习式索引才有机会赢。这正是一个"知道结论在什么条件下失效"的工程判断，比"用了新技术"更能说明问题。

...
注意：`llm/__init__.py` 导入 `.advisor` 与 `.tuning`，它们依赖 Task 3 / Task 4 的文件——**先建空占位文件** `llm/advisor.py`、`llm/tuning.py`、`llm/prompts.py`、`llm/cli.py`（每个只放模块 docstring），Task 3/4/5 再填充实现；否则 `llm/__init__.py` 无法导入。
